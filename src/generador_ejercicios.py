...
class GeneradorEjercicios:
    ...
    def _generar_z(self, crit, ej_id, params):
        ...
        if crit == 1:
            ...
            e["resolucion"] = (
                f"**Paso 1: Identificación y modelo.**\n"
                f"Para $Z \\sim \\mathcal{{N}}(0, 1)$, cola superior desde el valor negativo $-a = -{a:.2f}$.\n\n"
                f"**Paso 2: Propiedad de simetría.**\n"
                f"Por la simetría de la campana en torno a cero:\n"
                f"$$P(Z \\ge -{a:.2f}) = P(Z \\le {a:.2f})$$\n\n"
                f"**Paso 3: Consulta en la Tabla Normal Estándar.**\n"
                f"Fila `{r_k}`, columna `{c_k}`: $P(Z \\le {a:.2f}) = {prob:.4f}$.\n\n"
                f"**Paso 4: Conclusión.**\n"
    ...
    def _generar_t(self, crit, ej_id, params):
        ...
        if crit == 1:
            ...
            e["resolucion"] = (
                ...
                f"$$P(T_{{{df}}} \\ge -{a:.3f}) = P(T_{{{df}}} \\le {a:.3f})$$\n\n"
                f"**Paso 2: Conclusión.**\n"
